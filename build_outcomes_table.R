  ...
  df <- data.frame(
            ...
            c(#labels depend on whether or not the country includes relapses with new
               ifelse(NZ(pdata()$profile_data[, "rel_with_new_flg"]) == 1,
                      paste("|People with a new or relapse case of TB started on treatment for TB in|", dcyear - 2),
               ...
               ifelse(NZ(pdata()$profile_data[, "rel_with_new_flg"]) == 1,
                      paste("|People with a previously treated case of TB (but not a relapse case) started on treatment for TB in|", dcyear - 2),
                      #paste(ltxt(plabs(), "tsr_ret_nrel"), dcyear - 2),
                      paste("|People with a previously treated case of TB started on treatment for TB in|", dcyear - 2)
                      #paste(ltxt(plabs(), "tsr_allret"), dcyear - 2)
                      ),

               paste("|People with a new or relapse case of TB who are living with HIV started on treatment for TB in|", dcyear - 2),
               paste("|People started on treatment for TB that is resistant to rifampicin (RR/MDR-TB) in|", dcyear - 2),
               paste("|People started on treatment for TB that is resistant to both rifampicin and fluoroquinolones (pre-XDR-TB/XDR-TB) in|", dcyear - 2)
